...
const RichTooltip = ({
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-background-input border border-strong rounded-xl p-3 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.8)] min-w-[260px] animate-in fade-in zoom-in-95 duration-150 relative z-50">
        {/* Header */}
        <div className="flex justify-between items-end border-b border-subtle pb-2 mb-2">
        ...
        {benchmarkMode === "USD" && (
          <div className="grid grid-cols-[1fr_auto_auto] gap-x-4 gap-y-1.5">
            {sortedBreakdown.map((asset: any) => {
              // PnL Color Logic
              let pnlColor = "text-content-tertiary"; // Default Gray for 0%
              ...
              return (
                <React.Fragment key={asset.name}>
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    <div
                      className="w-1.5 h-1.5 rounded-full shrink-0 shadow-[0_0_6px_rgba(0,0,0,0.5)]"
                      style={{ backgroundColor: asset.color }}
                    />
                    <span className="text-[10px] font-bold text-content-secondary truncate">
                      {asset.name}
                    </span>
                  </div>
                  ...
                  {/* Col 3: PnL % */}
                  <div
                    className={`text-[10px] font-mono font-bold text-right tabular-nums w-[42px] ${pnlColor}`}
                  >
...
const BalanceChart: React.FC<BalanceChartProps> = ({
  ...
  const chartData = useMemo(() => {
    ...
    const TARGET_BTC = 63200.0;
    const TARGET_BNB = 592.5;
    const TARGET_SOL = 142.5;
    const TARGET_ARB = 1.12;
    const TARGET_PEPE = 0.000012;
    const TARGET_STABLE = 1.0;

    // Start Prices (Simulated 7 days ago)
    const START_ETH = 3050.0;
    const START_BTC = 60000.0;
    const START_BNB = 585.0;
    const START_SOL = 138.0;
    const START_ARB = 1.05;
    const START_PEPE = 0.00001;
    ...
    for (let i = 0; i <= totalPoints; i++) {
      const date = new Date(
        now.getTime() -
          (totalPoints - i) * (24 / pointsPerDay) * 60 * 60 * 1000,
      );
      ...
      const pBtc = interpolate(START_BTC, TARGET_BTC);
      ...
      const total = vEth + vBtc + vBnb + vUsdt + vSol + vArb + vPepe + vUsdc;
      ...
      const breakdown = [
        {
          name: "BTC",
          value: vBtc,
          change: ((pBtc - START_BTC) / START_BTC) * 100,
          color: "#F7931A",
        },
        {
          name: "BNB",
          value: vBnb,
          change: ((pBnb - START_BNB) / START_BNB) * 100,
          color: "#F3BA2F",
        },
        { name: "USDT", value: vUsdt, change: 0, color: "#26A17B" }, // Stable
        {
          name: "ETH",
          value: vEth,
          change: ((pEth - START_ETH) / START_ETH) * 100,
          color: "#627EEA",
        },
        {
          name: "SOL",
          value: vSol,
          change: ((pSol - START_SOL) / START_SOL) * 100,
          color: "#14F195",
        },
        {
          name: "PEPE",
          value: vPepe,
          change: ((pPepe - START_PEPE) / START_PEPE) * 100,
          color: "#4C9540",
        },
        {
          name: "ARB",
          value: vArb,
          change: ((pArb - START_ARB) / START_ARB) * 100,
          color: "#28A0F0",
        },
      ...
      if (benchmarkMode !== "USD") {
        ...
        benchmarkPlotValue =
          ((benchmarkPrice - startBenchmark) / startBenchmark) * 100;
      }
