...
const AUDIT_LOGS = [
  {
    id: 1,
    action: "ROLE_CHANGE",
    desc: "Promoted Sarah Chen to Senior Trader",
    user: "Alexander V.",
    time: "2 days ago",
  },
  {
    id: 2,
    action: "LIMIT_UPDATE",
    desc: "Increased Junior Trader Limit to $10k",
    user: "Alexander V.",
    time: "5 days ago",
  },
  {
    id: 3,
    action: "USER_REVOKE",
    desc: "Revoked access for J. Doe (Terminated)",
    user: "Alexander V.",
    time: "1 week ago",
  },
  // Critical Security Event
  {
    ...
    desc: "Mike Ross attempted trade on non-whitelisted protocol (SushiSwap)",
    user: "System (Policy Engine)",
    time: "1 hour ago",
  },
];
...
const TeamSettingsView: React.FC = () => {
  ...
  const handleRevoke = (id: string) => {
    if (
      confirm(
        "Are you sure? This will instantly terminate their active session and invalidate their MPC shard keys.",
      )
    ) {
      setMembers((prev) =>
  ...
  const getRoleBadge = (role: Role) => {
    switch (role) {
      ...
      default:
        return "bg-background-surface text-content-secondary";
    }
  };
  ...
  const getLimitDisplay = (member: TeamMember) => {
    if (member.role === "ADMIN")
      return (
        <span className="text-xs font-black text-purple-400 uppercase tracking-widest shadow-purple-500/20 text-shadow-glow">
          UNLIMITED
        </span>
      );
    if (member.role === "CFO")
      return (
        <span className="text-xs font-black text-content-tertiary uppercase tracking-widest">
          READ ONLY
        </span>
      );
      ...
      <div className="max-w-[1700px] mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h1 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-3">
              <Users className="text-content-secondary" />
            ...
            <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-emerald-500/5 border border-emerald-500/20 rounded-lg">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[9px] text-content-tertiary">
                  Private keys are sharded. No single user access.
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          ...
          <div className="xl:col-span-8 space-y-6">
            <div className="bg-zinc-900/40 border border-subtle rounded-[2rem] p-8 shadow-xl backdrop-blur-md overflow-hidden">
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  className="text-[10px] text-content-tertiary font-bold bg-background-card px-3 py-1 rounded-full border border-subtle flex items-center gap-2 hover:border-active hover:text-content-secondary transition-all cursor-pointer group"
                  title="Manage Plan & Billing"
                >
                  <span>
                    {members.filter((m) => m.status === "ACTIVE").length} / 10
                    Seats Used
                  </span>
                  <CreditCard
                    size={10}
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {members.map((member) => (
                      <tr
                        key={member.id}
                        className={`group transition-colors ${member.status === "REVOKED" ? "bg-rose-500/5" : "hover:bg-white/[0.02]"}`}
                      >
                        <td className="py-5 pl-4">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-10 h-10 rounded-xl overflow-hidden border ${member.status === "REVOKED" ? "border-rose-500 grayscale opacity-50" : "border-strong"}`}
                            >
                              <img
                                src={member.avatar}
                                alt={member.name}
                            ...
                            <div>
                              <div
                                className={`text-sm font-bold ${member.status === "REVOKED" ? "text-content-tertiary line-through" : "text-white"}`}
                              >
                                {member.name}
                              </div>
                        ...
                        <td className="py-5">
                          <div className="flex flex-col items-start gap-2">
                            <span
                              className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-wider border ${getRoleBadge(member.role)}`}
                            >
                        ...
                        <td className="py-5">{getLimitDisplay(member)}</td>
                        <td className="py-5">
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2 text-[10px] font-medium text-content-secondary">
                              <Smartphone
                                size={12}
                                className={
                                  member.mfaEnabled
                              ...
                              />
                              <span>
                                2FA: {member.mfaEnabled ? "Enabled" : "Off"}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 text-[10px] font-medium text-content-secondary">
                              <Key
                        ...
                        <td className="py-5 text-right pr-4">
                          {member.role !== "ADMIN" &&
                            member.status !== "REVOKED" && (
                              <div className="flex justify-end gap-2">
                                <button
                                  className="p-2 bg-background-surface hover:bg-zinc-700 text-content-secondary hover:text-white rounded-lg border border-subtle transition-colors"
                          ...
                          {member.status === "REVOKED" && (
                            <button className="text-[10px] font-bold text-content-tertiary uppercase hover:text-white transition-colors">
                              Re-Invite
                            </button>
                          )}
                        </td>
            ...
            {/* AUDIT LOGS */}
            <div className="bg-zinc-900/40 border border-subtle rounded-[2rem] p-8 shadow-xl backdrop-blur-md">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-[10px] font-black text-content-tertiary uppercase tracking-[0.2em]">
                  Admin Activity Log
                </h3>
                    ...
                    <div
                      ...
                      className={`flex items-center justify-between p-4 border rounded-xl transition-colors ${
                        isCritical
                          ...
                          : "bg-zinc-900/30 border-subtle hover:bg-zinc-900/50"
                      }`}
                    >
                      <div className="flex items-center gap-4">
                        <div
                        ...
                        <div>
                          ...
                          <p
                            className={`text-[10px] font-medium ${isCritical ? "text-rose-400/80" : "text-content-tertiary"}`}
                          >
                            {log.desc}
                          </p>
                        </div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[9px] font-bold text-content-tertiary uppercase tracking-wider">
                          {log.time}
                        </p>
                      </div>
                    </div>
          ...
          <div className="xl:col-span-4 space-y-6">
            ...
            <div className="bg-gradient-to-br from-emerald-900/20 to-zinc-900 border border-emerald-500/20 rounded-[2rem] p-6 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs text-content-secondary leading-relaxed mb-4">
                  Your team members do <strong>not</strong> have private keys.
                  They authenticate via Web3 Auth (Email/SSO) which decrypts a
                  local MPC shard.
                </p>
                <div className="bg-black/40 rounded-xl p-4 border border-emerald-500/10">
                  ...
                  <p className="text-[10px] text-content-secondary">
                    If you revoke a user, their shard is instantly invalidated
                    by the policy engine. They cannot move funds even if they
                    saved their browser session.
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* 2. ROLE DEFINITIONS */}
            <div className="bg-zinc-900/40 border border-subtle rounded-[2rem] p-6 shadow-xl backdrop-blur-md">
              <h3 className="text-[10px] font-black text-content-tertiary uppercase tracking-[0.2em] mb-6">
                Role Policies
              </h3>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl border border-blue-500/10 bg-blue-500/5">
                  ...
                  <ul className="space-y-2">
                    <li className="flex items-center gap-2 text-[10px] text-content-secondary">
                      <CheckCircle2 size={10} className="text-blue-500" /> Max
                      Trade: $10,000 / day
                    </li>
                    <li
                      className="flex items-center gap-2 text-[10px] text-content-secondary cursor-help"
                      title="Can only interact with approved smart contracts."
                    >
                    ...
                    <li className="flex items-center gap-2 text-[10px] text-content-secondary">
                      <CheckCircle2 size={10} className="text-blue-500" />{" "}
                      Cannot initiate transfers to external addresses
                    </li>
                  </ul>
                </div>
                ...
                <div className="p-4 rounded-xl border border-amber-500/10 bg-amber-500/5">
                  ...
                  <ul className="space-y-2">
                    <li className="flex items-center gap-2 text-[10px] text-content-secondary">
                      <CheckCircle2 size={10} className="text-amber-500" /> View
                      All Wallets
                    </li>
                    <li className="flex items-center gap-2 text-[10px] text-content-secondary">
                    ...
                    <li className="flex items-center gap-2 text-[10px] text-content-tertiary">
