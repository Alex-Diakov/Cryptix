  ...
  return createPortal(
    <div className="relative z-[9999]">
      ...
      >
        ...
        <div className="flex-shrink-0 pt-12 px-8 pb-6 border-b border-subtle relative bg-zinc-900/80 backdrop-blur-xl">
          <button
            onClick={onClose}
        ...
        {/* Content (Scrollable) */}
        <div className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-hide bg-background-card">
          {/* On-Chain Analysis Block */}
          <div className="bg-background-card border border-subtle p-5 rounded-lg relative overflow-hidden group">
            <div className="flex items-start space-x-3 relative z-10">
              ...
              <div>
                ...
                <p className="text-[11px] text-content-secondary font-medium leading-relaxed">
                  ...
                  <span className="text-white font-bold">ETH</span> detected.
                  Large whales are withdrawing from CEX to{" "}
                  <span className="text-white font-bold">Cold Storage</span>{" "}
                  (bullish signal).
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1 border-b border-subtle pb-2">
            ...
            {WHALE_ACTIVITY.map((whale) => (
              <div
                key={whale.id}
                className="bg-zinc-900/30 border border-subtle p-4 rounded-lg hover:bg-zinc-900/50 transition-all group relative overflow-hidden"
              >
                {/* Header: Entity Info */}
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 rounded-lg overflow-hidden bg-background-card border border-strong p-0.5">
                      <img
                        src={whale.avatar}
                        alt={whale.entityName}
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-0.5">
                        <span className="text-[9px] font-bold text-content-tertiary uppercase tracking-widest">
                          {whale.metricLabel}:{" "}
                  ...
                  <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      title="Copy"
                      className="p-1.5 bg-background-surface rounded hover:text-white transition-colors text-content-tertiary"
                    >
                      <Copy size={10} />
                    </button>
                  </div>
                ...
                <div className="flex items-baseline space-x-2 mb-1">
                  <div
                    className={`flex items-center space-x-1.5 text-xs font-bold uppercase tracking-wide ${getActionColor(whale.actionType)}`}
                  >
                ...
                {/* Destination */}
                {(whale.actionType === "BRIDGE" ||
                  whale.actionType === "STAKE") &&
                  whale.destination && (
                    <div className="text-[9px] text-content-tertiary font-mono mb-1 flex items-center gap-1">
                      <span>to</span>
        ...
        {/* Footer Action */}
        <div className="flex-shrink-0 p-8 border-t border-subtle bg-zinc-950/90 backdrop-blur-xl">
          <button
            ...
            className={`w-full py-4 rounded-lg font-black text-[10px] uppercase tracking-[0.2em] transition-all active:scale-[0.98] flex items-center justify-center space-x-3 ${
              isTracking
                ...
                : "bg-transparent border border-active text-white hover:border-emerald-500 hover:text-emerald-500"
            }`}
          >
            {isTracking ? (
              <>
