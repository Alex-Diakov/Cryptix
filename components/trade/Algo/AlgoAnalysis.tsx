    ...
    <section className="lg:col-span-6 bg-zinc-900/40 border border-subtle rounded-[2.5rem] p-8 flex flex-col h-full overflow-hidden relative">
      ...
      <div className="flex flex-col gap-6 flex-1 overflow-y-auto scrollbar-hide">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-background-surface border border-subtle rounded-2xl p-4 flex flex-col justify-center relative overflow-hidden group">
            <span className="text-[9px] font-bold text-content-tertiary uppercase tracking-widest mb-2 relative z-10">
              Impact Saved
            </span>
            <div className="flex items-baseline gap-1 relative z-10">
              <span
                className={`text-2xl font-mono font-bold ${numericPay > 0 ? "text-emerald-400" : "text-content-tertiary"}`}
              >
                {numericPay > 0
                  ? `+${formatMoney(simulation.instantImpactCost)}`
                  : "$0.00"}
              </span>
            </div>
            <div className="text-[9px] text-content-tertiary mt-1 relative z-10">
              vs Market Dump
            </div>
            {/* Decor */}
          ...
          <div className="bg-background-surface border border-subtle rounded-2xl p-4 flex flex-col justify-center relative overflow-hidden group">
            ...
            <div className="flex items-baseline gap-1 relative z-10">
              ...
              >
                {numericPay > 0
                  ? `+${formatMoney(simulation.netSavings)}`
                  : "$0.00"}
              </span>
            </div>
            <div className="text-[9px] text-emerald-500/70 mt-1 relative z-10">
              Net of All Fees
            </div>
          </div>

          {/* Card 3: Total Cost */}
          <div className="bg-background-surface border border-subtle rounded-2xl p-4 flex flex-col justify-center">
            <span className="text-[9px] font-bold text-content-tertiary uppercase tracking-widest mb-2">
              Total Algo Cost
            </span>
            <span className="text-2xl font-mono font-bold text-content-secondary">
        ...
        <div className="bg-background-surface border border-subtle rounded-2xl p-5 flex flex-col flex-1 min-h-[240px] overflow-hidden">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              className="text-[9px] font-bold text-content-tertiary hover:text-white uppercase tracking-wider flex items-center gap-1 transition-colors"
            >
              Strategy Parameters{" "}
              <ChevronDown
                size={10}
          ...
          <div
            className={`grid grid-cols-2 gap-2 overflow-hidden transition-all duration-300 ease-in-out mb-4 ${isDetailsOpen ? "max-h-40 opacity-100 pb-4 border-b border-subtle" : "max-h-0 opacity-0"}`}
          >
            <div className="bg-zinc-900/50 p-3 rounded-xl border border-subtle flex flex-col justify-center">
              <div className="flex items-center gap-1.5 text-content-tertiary mb-1">
                <Activity size={12} />{" "}
                <span className="text-[9px] uppercase font-bold">Variance</span>
              </div>
              <span className="text-xs font-mono font-bold text-white">
                {algoRandomize ? "High (Randomized)" : "Low (Fixed)"}
              </span>
            </div>
            <div className="bg-zinc-900/50 p-3 rounded-xl border border-subtle flex flex-col justify-center">
              <div className="flex items-center gap-1.5 text-content-tertiary mb-1">
