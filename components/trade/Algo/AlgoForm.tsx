...
export const AlgoForm: React.FC<AlgoFormProps> = ({
  amount,
  onAmountChange,
  balance,
  logic,
  ...
  const {
    ...
    icebergVisibleAmount,
    setIcebergVisibleAmount,
    isAlgoRunning,
    setIsAlgoRunning,
    executionProgress,
    simulation,
    ...
    return (
      <section className="lg:col-span-4 flex flex-col h-full gap-5">
        <div className="glass-panel p-2 h-full flex flex-col">
          <div className="bg-background-surface rounded-[2.2rem] p-6 border border-subtle flex flex-col h-full relative overflow-hidden">
            {/* Background Pulse */}
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
              ...
              <span className="text-[10px] font-mono font-bold text-content-secondary tabular-nums">
                {algoStrategy} • {simulation.numTranches} Tranches
              </span>
            </div>

            <div className="flex-1 flex flex-col justify-center relative z-10 space-y-8">
              <div className="text-center">
                <p className="text-[10px] font-bold text-content-tertiary uppercase tracking-widest mb-2">
                  Filled Amount
                </p>
                <p className="text-5xl font-mono font-medium text-white tabular-nums tracking-tighter">
                  {formatToken(
                    (parseFloat(amount) || 0) * (executionProgress / 100),
                  )}{" "}
                  <span className="text-content-tertiary text-3xl">ETH</span>
                </p>
              </div>

              <div className="space-y-2">
                <div className="flex justify-between text-[9px] font-mono text-content-tertiary uppercase tracking-wider">
                  <span>Progress</span>
                  <span>{executionProgress.toFixed(1)}%</span>
                </div>
                <div className="h-4 w-full bg-background-subtle rounded-full overflow-hidden border border-subtle p-0.5">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-emerald-600 to-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.4)] transition-all duration-300 ease-out"
                    style={{ width: `${executionProgress}%` }}
                  ></div>
                </div>
                <div className="flex justify-center mt-2">
                  <span className="text-[9px] font-mono text-content-secondary bg-background-card px-2 py-1 rounded">
                    Est. Time Remaining:{" "}
                    <span className="text-white">
                      {Math.ceil(
                        simulation.avgIntervalMins *
                          (simulation.numTranches *
                            (1 - executionProgress / 100)),
                      )}
                      m
                    </span>
                  </span>
    ...
    <section className="lg:col-span-4 flex flex-col h-full gap-5">
      ...
      <div className="glass-panel p-2">
        <div className="bg-background-surface rounded-[2.2rem] p-6 border border-subtle space-y-2">
          ...
          <div className="bg-background-subtle rounded-3xl p-5 border border-subtle focus-within:border-emerald-500/30 transition-colors group relative">
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                placeholder="0.00"
              />
              <div className="flex items-center gap-2 bg-background-card border border-strong pl-2 pr-3 py-2 rounded-xl shrink-0">
          ...
          <div className="bg-background-subtle rounded-3xl p-5 border border-subtle space-y-6">
            {/* Strategy Selector */}
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <Settings size={12} className="text-content-tertiary" />
              </div>
              <div className="flex bg-zinc-900/80 p-1 rounded-xl border border-subtle">
                {["TWAP", "VWAP", "ICEBERG"].map((strat) => (
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${
                      algoStrategy === strat
                        ? "bg-background-surface text-white shadow-lg border border-strong"
              ...
              <div className="space-y-4 animate-in fade-in">
                <div>
                  <div className="flex justify-between mb-2">
                    <label className="text-[9px] font-bold text-content-tertiary uppercase tracking-wide">
                      Duration
                    </label>
                  ...
                  <input
                    ...
                    value={algoDuration}
                    onChange={(e) =>
                      setAlgoDuration(parseFloat(e.target.value))
                    }
                    className="w-full h-1.5 bg-background-surface rounded-full appearance-none cursor-pointer accent-emerald-500 hover:accent-emerald-400"
                  />
                </div>
                <div
                  onClick={() => setAlgoRandomize(!algoRandomize)}
                  className="flex items-center justify-between p-3 bg-zinc-900/50 border border-subtle rounded-xl cursor-pointer hover:bg-background-card transition-colors"
                >
                  <span className="text-[9px] font-bold text-content-secondary uppercase tracking-wide">
                    Randomize Intervals
                  </span>
                  <div
                    className={`w-8 h-4 rounded-full relative transition-colors ${algoRandomize ? "bg-emerald-500" : "bg-zinc-700"}`}
                  >
                    <div
                      className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${algoRandomize ? "left-4.5" : "left-0.5"}`}
                    ></div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 animate-in fade-in">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setParticipationRate(parseFloat(e.target.value))
                    }
                    className="w-full h-1.5 bg-background-surface rounded-full appearance-none cursor-pointer accent-emerald-500"
                  />
                </div>
              </div>
            )}
            ...
            {algoStrategy === "ICEBERG" && (
              <div className="space-y-4 animate-in fade-in">
                <div className="bg-zinc-900/50 border border-subtle rounded-xl p-3 flex flex-col gap-1 focus-within:border-emerald-500/30 transition-colors">
