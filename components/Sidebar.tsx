...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <aside className="hidden md:flex w-[72px] h-full bg-background-card border-r border-subtle flex-col shrink-0 z-50 transition-all duration-300">
      {/* 2. Logo */}
            ...
            >
              <button
                ...
                className={`w-10 h-10 flex items-center justify-center rounded-lg transition-all relative ${
                  activeTab === item.id
                    ? "bg-white/10 text-white border border-subtle shadow-inner"
                    : "text-content-tertiary hover:text-zinc-200 hover:bg-white/[0.03]"
                }`}
              >
                {activeTab === item.id && (
              ...
              <div className="absolute left-full ml-3 px-2 py-1 bg-background-surface border border-strong rounded text-[10px] font-medium text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-xl">
                {item.label}
              </div>
            </li>
          ))}
      ...
      {/* 4. Bottom Actions */}
      <div className="pb-4 px-2 space-y-3 flex flex-col items-center border-t border-subtle pt-4">
        {/* Settings Button with Dropdown (Unified Design) */}
        <div className="relative group">
          <button
            onClick={() => setIsSettingsOpen(!isSettingsOpen)}
            className={`w-10 h-10 flex items-center justify-center rounded-lg transition-all ${isSettingsOpen ? "bg-emerald-500 text-black shadow-lg shadow-emerald-500/20" : "text-content-tertiary hover:text-white hover:bg-white/[0.05]"}`}
          >
            <Settings size={18} />
          </button>
          ...
          {isSettingsOpen && (
            <>
              ...
              <div className="absolute left-full bottom-0 ml-3 w-64 bg-background-card border border-strong rounded-2xl shadow-[0_20px_50px_-10px_rgba(0,0,0,0.8)] overflow-hidden z-50 animate-in fade-in slide-in-from-left-2 duration-200">
                {/* Header matching Profile */}
                <div className="p-4 border-b border-subtle bg-zinc-900/50">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-background-surface flex items-center justify-center text-content-secondary border border-subtle">
                      <Building size={20} />
                    </div>
                    <div>
                ...
                <div className="p-2 space-y-1">
                  <div className="px-2 py-1.5 text-[9px] font-black text-content-tertiary uppercase tracking-widest">
                    Global Controls
                  </div>
                  ...
                  {settingsOptions.map((opt) => (
                    ...
                    >
                      <div
                        className={`text-content-secondary group-hover:${opt.color} transition-colors`}
                      >
                        <opt.icon size={16} />
                      </div>
                      <div className="flex-1">
                        <div className="text-[11px] font-bold text-zinc-200 group-hover:text-white">
                          {opt.label}
                        </div>
                        <div className="text-[9px] text-content-tertiary group-hover:text-content-secondary">
                          {opt.desc}
                        </div>
                      </div>
                      <ChevronRight
                        size={12}
                        className="text-zinc-700 group-hover:text-content-tertiary"
                      />
                    </button>
                  ))}
                </div>
                ...
                {/* Footer Context */}
                <div className="p-2 border-t border-subtle bg-zinc-900/30 text-center">
                  <span className="text-[9px] text-content-tertiary font-mono">
                    Build v4.1.0 (Enterprise)
                  </span>
                </div>
              </div>
            </>
        ...
        <div className="relative group">
          <button className="w-10 h-10 flex items-center justify-center rounded-lg text-content-tertiary hover:text-rose-500 hover:bg-rose-500/10 transition-all">
            <LogOut size={18} />
          </button>
          {/* Tooltip for Logout */}
          <div className="absolute left-full ml-3 px-2 py-1 bg-background-surface border border-strong rounded text-[10px] font-medium text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-xl">
