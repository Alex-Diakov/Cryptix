...
const MobileNav: React.FC<MobileNavProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-xl border-t border-strong px-6 pb-6 pt-3 z-50 flex items-end justify-between">
      {navItems.map((item) => {
        if (item.isAction) {
          return (
            <button
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase mt-2 text-content-tertiary">
                {item.label}
              </span>
            </button>
          );
