...
const ActivityLogView: React.FC = () => {
      ...
      case "PENDING":
        return (
          <button className="group relative inline-flex items-center gap-1.5 px-3 py-1.5 rounded bg-amber-500/10 text-amber-500 border border-amber-500/20 text-[10px] font-black uppercase tracking-wider hover:bg-amber-500/20 transition-all cursor-pointer shadow-sm hover:shadow-amber-500/10">
            ...
            {/* Actionable Tooltip */}
            <div className="absolute right-0 top-full mt-2 w-64 p-4 bg-background-surface border border-subtle rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,1)] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 text-left translate-y-1">
              <div className="flex items-start gap-3">
                <div className="p-1.5 bg-amber-500/10 rounded-lg text-amber-500 mt-0.5">
                  <ShieldAlert size={14} />
                </div>
                <div>
                  <p className="text-[10px] text-content-secondary font-medium leading-relaxed normal-case mb-2">
              ...
              {/* Arrow */}
              <div className="absolute -top-1.5 right-4 w-3 h-3 bg-background-surface border-t border-l border-subtle rotate-45"></div>
            </div>
          </button>
        );
      default:
  ...
  const getRoleIcon = (role: ActorRole) => {
    switch (role) {
      ...
      case "BOT":
        return <Bot size={12} className="text-content-secondary" />;
      case "TRADER":
        return <User size={12} className="text-blue-400" />;
      case "CFO":
        return <FileCode size={12} className="text-amber-400" />;
    }
  };
  ...
  const getRowBackground = (status: Status) => {
    switch (status) {
      case "FAILED":
        return "bg-rose-500/[0.08] hover:bg-rose-500/[0.12]";
      case "PENDING":
        return "bg-amber-500/[0.06] hover:bg-amber-500/[0.1]";
      default:
  ...
  return (
    <div className="h-full overflow-hidden flex flex-col p-6 lg:p-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="max-w-[1700px] mx-auto w-full h-full flex flex-col">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 shrink-0">
          <div>
            <h1 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-3">
              <ShieldAlert className="text-content-secondary" />
              Audit Log & Activity
            </h1>
            <p className="text-content-secondary text-xs font-medium mt-1">
              Immutable record of system events, security alerts, and user
              actions.
            </p>
          </div>

          <div className="flex items-center gap-3">
            <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-background-card border border-strong text-content-secondary hover:text-white hover:border-active transition-all text-[10px] font-bold uppercase tracking-wider">
              <FileCode size={14} />
        ...
        {/* 2. FILTER TOOLBAR */}
        <div className="flex flex-col gap-4 mb-4 shrink-0 bg-background-surface p-2 rounded-xl border border-subtle">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              <button className="flex items-center gap-2 px-3 py-2 bg-background-card border border-strong rounded-lg text-xs font-medium text-content-secondary hover:text-white hover:border-active transition-all">
                <Calendar size={14} className="text-content-tertiary" />
                <span>Last 30 Days</span>
                <ChevronDown size={12} className="ml-1 text-content-tertiary" />
              </button>
            </div>

            {/* Right Group: Filters */}
            <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto justify-start md:justify-end">
              <div className="flex bg-background-card rounded-lg p-1 border border-subtle gap-1 w-full md:w-auto">
                <button
                  onClick={() => handleFilterClick("Actor")}
                  className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-[9px] font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${activeFilter?.type === "Actor" ? "bg-background-surface text-white" : "text-content-secondary hover:text-white hover:bg-background-surface"}`}
                >
                  <User size={12} /> <span>Actor</span>{" "}
                  <ChevronDown size={10} />
                </button>
                <div className="w-px h-4 bg-white/5 my-auto hidden md:block"></div>
                <button className="flex-1 md:flex-none px-3 py-1.5 rounded-md text-[9px] font-bold uppercase tracking-wider text-content-secondary hover:text-white hover:bg-background-surface transition-all flex items-center justify-center gap-2">
                  <FileCode size={12} /> <span>Event</span>{" "}
                  <ChevronDown size={10} />
                </button>
                <div className="w-px h-4 bg-white/5 my-auto hidden md:block"></div>
                <button className="flex-1 md:flex-none px-3 py-1.5 rounded-md text-[9px] font-bold uppercase tracking-wider text-content-secondary hover:text-white hover:bg-background-surface transition-all flex items-center justify-center gap-2">
                  <Filter size={12} /> <span>Status</span>{" "}
                  <ChevronDown size={10} />
                </button>
              </div>
            </div>
          ...
          {activeFilter && (
            <div className="flex items-center gap-2 animate-in fade-in slide-in-from-top-1 px-1 pb-1">
              <span className="text-[10px] text-content-tertiary font-bold uppercase tracking-widest">
                Filtering by:
              </span>
        ...
        {/* 3. DATA TABLE */}
        <div className="flex-1 bg-zinc-900/40 border border-subtle rounded-[2rem] overflow-hidden shadow-2xl backdrop-blur-md relative flex flex-col">
          {/* Table Header */}
          <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-subtle bg-zinc-900/80 sticky top-0 z-10 text-[10px] font-black uppercase tracking-[0.2em] text-content-tertiary">
              ...
              {AUDIT_DATA.map((row) => (
                ...
                >
                  ...
                  <div className="col-span-2 pl-2 flex items-center gap-2">
                    <span className="font-mono text-xs text-content-tertiary group-hover:text-content-secondary transition-colors">
                      {row.timestamp}
                    </span>
                  </div>
                  ...
                  <div className="col-span-2 flex items-center gap-3">
                    {row.actor.avatar ? (
                      <img
                        ...
                        alt="avatar"
                        className="w-6 h-6 rounded-full bg-background-surface"
                      />
                    ) : (
                      <div className="w-6 h-6 rounded-full bg-background-surface flex items-center justify-center border border-subtle">
                        <User size={12} className="text-content-tertiary" />
                      </div>
                    )}
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[9px] font-bold text-content-tertiary uppercase">
                          {row.actor.role}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="col-span-2">
                    <span
                      className={`text-xs font-bold ${row.details.isRisk ? "text-white" : "text-content-secondary"}`}
                    >
                      {row.event}
                    </span>
                  </div>
                  ...
                  <div className="col-span-3">
                    <div className="flex flex-col">
                      <span
                        className={`text-[11px] ${row.details.isRisk ? "font-semibold text-zinc-200" : "font-medium text-content-secondary"} group-hover:text-content-secondary transition-colors`}
                      >
                        {row.details.label}
                      </span>
                      {row.details.value && (
                  ...
                  <div className="col-span-2">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-1.5 text-xs font-mono text-content-tertiary hover:text-blue-400 transition-colors group/link w-fit"
                    >
                      <span className="text-blue-500/80 group-hover/link:text-blue-400">
                        {row.refHash}
                      </span>
                      <ExternalLink
                        size={10}
                        className="text-content-tertiary group-hover/link:text-blue-400 transition-colors"
                      />
                    </a>
                  </div>
