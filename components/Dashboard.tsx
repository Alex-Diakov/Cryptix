...
const Dashboard: React.FC<DashboardProps> = ({ onTradeClick }) => {
  ...
  const [benchmarkMode, setBenchmarkMode] = useState<"USD" | "ETH" | "BTC">(
    "USD",
  );
  ...
  const { totalBalance, totalCostBasis, cryptoTotals } = useMemo(() => {
    return MOCK_ASSETS.reduce(
      (acc, asset) => ({
        totalBalance: acc.totalBalance + asset.value,
        totalCostBasis:
          acc.totalCostBasis + asset.avgPurchasePrice * asset.quantity,
        cryptoTotals: {
          ETH:
            acc.cryptoTotals.ETH +
            (asset.symbol === "ETH" ? asset.quantity : 0),
          BTC:
            acc.cryptoTotals.BTC +
            (asset.symbol === "BTC" ? asset.quantity : 0),
        },
      }),
  ...
  const totalPnLPercent =
    totalCostBasis > 0 ? (totalPnLValue / totalCostBasis) * 100 : 0;
  ...
  const pnlData = useMemo(() => {
    ...
    switch (timeRange) {
      ...
      case "ALL":
        pct = totalPnLPercent;
        break; // Use REAL calculated PnL for 'ALL'
      default:
        pct = 2.1;
    }
    ...
    return {
      val:
        (displayVal >= 0 ? "+" : "-") +
        "$" +
        Math.abs(displayVal).toLocaleString("en-US", {
  ...
  const displayBalance = useMemo(() => {
    ...
    if (benchmarkMode === "ETH")
      return (
        (totalBalance / ETH_PRICE).toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }) + " ETH"
      );
    if (benchmarkMode === "BTC")
      return (
        (totalBalance / BTC_PRICE).toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }) + " BTC"
      );
    ...
    <div className="flex flex-col lg:flex-row h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto scrollbar-hide p-6 lg:p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-3 min-h-[420px] bg-zinc-900/40 border border-subtle rounded-[2.5rem] p-8 shadow-xl backdrop-blur-md flex flex-col relative group overflow-hidden">
            ...
            <div className="flex flex-col gap-5 relative z-10 mb-6 w-full">
              ...
              <div className="flex justify-between items-end w-full">
                <p className="text-[10px] font-black text-content-secondary uppercase tracking-[0.3em] ml-1 mb-1">
                  TOTAL BALANCE
                </p>
                ...
                <div className="flex gap-2 overflow-x-auto max-w-full pb-1">
                  {/* Benchmark Toggle */}
                  <div className="flex p-1 bg-black/40 rounded-xl border border-subtle shrink-0">
                    {["USD", "ETH", "BTC"].map((b) => (
                      <button
                        key={b}
                        onClick={() => setBenchmarkMode(b as any)}
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-black transition-all ${b === benchmarkMode ? "bg-background-surface text-white shadow-lg" : "text-content-tertiary hover:text-content-secondary"}`}
                      >
                        {b}
                      </button>
                    ))}
                  ...
                  <div className="flex p-1 bg-black/40 rounded-xl border border-subtle shrink-0">
                    {["1H", "1D", "1W", "1M", "1Y", "ALL"].map((t) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-black transition-all ${t === timeRange ? "bg-background-surface text-white shadow-lg" : "text-content-tertiary hover:text-content-secondary"}`}
                      >
                        {t}
                      </button>
                    ))}
              ...
              <div className="flex flex-col xl:flex-row justify-between items-end gap-4 w-full">
                ...
                <div className="flex items-baseline gap-4 flex-wrap">
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    />
                    <span
                      className={`text-lg font-mono font-medium tracking-tight ${isPositive ? "text-emerald-500" : "text-rose-500"}`}
                    >
                      {pnlData.val}{" "}
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1">
                  <button className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-white border border-strong pl-3 pr-4 py-2 rounded-xl transition-all active:scale-95 group shrink-0">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-emerald-500/5 hover:bg-emerald-500/10 text-emerald-500 border border-emerald-500/50 pl-3 pr-4 py-2 rounded-xl transition-all active:scale-95 group shrink-0"
                  >
            ...
            {/* Legend (Bottom) */}
            <div className="mt-4 pt-4 border-t border-subtle flex flex-wrap items-center justify-between text-[10px] font-bold uppercase tracking-widest text-content-tertiary relative z-10">
      ...
      <div className="w-full lg:w-[30%] lg:min-w-[350px] lg:max-w-[450px] bg-background-subtle border-l border-subtle overflow-y-auto scrollbar-hide p-6 space-y-6 shadow-[-10px_0_40px_-10px_rgba(0,0,0,0.5)] z-20 relative">
        ...
        <section className="bg-zinc-900/40 border border-subtle rounded-3xl shadow-2xl backdrop-blur-xl relative overflow-hidden group transition-all duration-500">
          <div className="p-6">
            {/* Top Header */}
            <div className="flex justify-between items-center mb-6 h-8">
              ...
              <button
                ...
                className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] font-black uppercase tracking-widest text-content-secondary hover:text-white transition-all border border-strong flex items-center space-x-2 disabled:opacity-50"
              >
                {isGenerating ? (
                  <>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-zinc-800/40 rounded-xl p-4 flex flex-col justify-between min-h-[100px] hover:bg-zinc-800/60 transition-colors">
                <div>
                  <span className="text-[10px] font-bold text-content-secondary uppercase tracking-widest mb-2 block opacity-70">
              ...
              <div className="bg-zinc-800/40 rounded-xl p-4 flex flex-col justify-between min-h-[100px] hover:bg-zinc-800/60 transition-colors">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[9px] font-bold text-emerald-500/80 mt-1 uppercase tracking-wider">
                    +8.2% APY
                  </span>
                </div>
              </div>
        ...
        <section className="bg-zinc-900/40 border border-subtle rounded-3xl p-6 flex flex-col hover:bg-zinc-900/60 transition-all backdrop-blur-sm relative overflow-hidden group">
          {/* Header */}
          <div className="flex items-center justify-between mb-6 h-8">
            <div className="flex items-center space-x-3">
              {/* TYPOGRAPHY UPDATE */}
              <h3 className="text-[11px] font-black text-content-secondary uppercase tracking-wide">
                SMART MONEY FLOW
              </h3>
            </div>
            ...
            <button
              ...
              className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] font-black uppercase tracking-widest text-content-secondary hover:text-white transition-all border border-strong flex items-center space-x-2"
            >
              <span>WALLETS</span>
              <ArrowRight size={12} className="translate-y-[1px]" />
            </button>
          ...
          <div className="flex items-start justify-between mb-8">
            <div className="flex flex-col items-start">
              <span className="text-5xl font-mono font-black text-white tracking-tighter leading-none mb-3">
                88%
              </span>
              <span className="text-xs font-bold text-content-secondary uppercase tracking-widest mb-2">
                SMART MONEY TRACKER
              </span>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-emerald-500 text-xs font-bold tracking-wide">
                  ACCUMULATING
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-start gap-4 pt-5 border-t border-subtle mt-auto">
            <span className="text-[10px] font-bold text-content-secondary uppercase tracking-widest whitespace-nowrap">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center -space-x-2">
                <img
                  ...
                  className="w-5 h-5 rounded-full border border-black bg-background-card p-0.5 relative z-10"
                  alt="ETH"
                />
                <img
                  src="https://cryptologos.cc/logos/arbitrum-arb-logo.png?v=035"
                  className="w-5 h-5 rounded-full border border-black bg-background-card p-0.5 relative z-20"
                  alt="ARB"
                />
              </div>
            </div>
        ...
        <section className="bg-zinc-900/40 border border-subtle rounded-3xl p-6 flex flex-col hover:bg-zinc-900/60 transition-all backdrop-blur-sm relative overflow-hidden group">
          ...
          <div className="mb-0 bg-zinc-800/30 p-4 rounded-xl border-none">
            ...
            <div className="flex items-center justify-start gap-4 mb-4">
              <span className="text-[10px] font-black text-content-tertiary uppercase tracking-widest">
                NET INFLOW 24H
              </span>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center space-x-2">
                <Coins size={14} className="text-content-secondary" />
              ...
              <div className="flex-1 h-px bg-gradient-to-r from-zinc-700 via-zinc-500 to-zinc-700 relative">
                <ArrowRight
                  size={12}
                  className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-content-secondary"
                />
              </div>
      ...
      <WhaleDrawer
        isOpen={isWhaleDrawerOpen}
        onClose={() => setIsWhaleDrawerOpen(false)}
      />
      <TokenDrawer
